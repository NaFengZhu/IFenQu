...
//			}
	...
	public static String[] getWholeAndDobNum(double price) {
		String lowPrice = String.valueOf(price);
		if (TextUtils.isEmpty(lowPrice))return new String[2];
		String[] result = lowPrice.split("\\.");
		if (result.length >= 2){
			String result2 = result[1];
			if (result2.length()>2){
				result[1] = result2.substring(0,2);
