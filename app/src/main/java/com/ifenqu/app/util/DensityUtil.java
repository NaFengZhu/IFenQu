...
public class DensityUtil {
    ...
    public static int dip2px(float dpValue) {
        final float scale = IfenquApplication.getInstance().getResources().getDisplayMetrics().density;
    ...
    public static void formartHight(View imageView, float bili, int type, int marginLR, int marginTop, int marginBottom) {
        WindowManager wm = (WindowManager) IfenquApplication.getInstance().getSystemService(Context.WINDOW_SERVICE);
    ...
    private boolean checkNavigator() {
        int id = IfenquApplication.getInstance().getResources().getIdentifier("config_showNavigationBar", "bool", "android");
        return id > 0 && IfenquApplication.getInstance().getResources().getBoolean(id);
    }
     ...
     * @return
     */
    private int measureHeightNavigator(){
        int resourceId = IfenquApplication.getInstance().getResources().getIdentifier("navigation_bar_height", "dimen", "android");
        if (resourceId > 0) {
            return IfenquApplication.getInstance().getResources().getDimensionPixelSize(resourceId);
        }
        return 0;
    }
